...
public class ExceptionEngine {
    ...
    public static ApiExcption handleException(Throwable e) {
        ApiExcption ex;
        if (e instanceof HttpException) {             //HTTP错误
            HttpException httpExc = (HttpException) e;
            ex = new ApiExcption("网络错误"+e.getMessage(), httpExc.code());
            //均视为网络错误
            return ex;
        } else if (e instanceof ServerException) {    //服务器返回的错误(交由开发者自己处理)
            ServerException serverExc = (ServerException) e;
            ex = new ApiExcption(serverExc.getMsg(),serverExc.getCode());
            return ex;
        } else if (e instanceof JsonParseException
                || e instanceof JSONException
                || e instanceof ParseException || e instanceof MalformedJsonException) {  //解析数据错误
            ex = new ApiExcption("网络解析错误"+e.getMessage(), ANALYTIC_SERVER_DATA_ERROR);
            return ex;
        } else if (e instanceof ConnectException || e instanceof SSLHandshakeException || e instanceof UnknownHostException) {//连接网络错误
            ex = new ApiExcption("连接失败"+e.getMessage(), CONNECT_ERROR);
            return ex;
        } else if (e instanceof SocketTimeoutException) {//网络超时
            ex = new ApiExcption("网络超时"+e.getMessage(), TIME_OUT_ERROR);
            return ex;
        } else {  //未知错误
            ex = new ApiExcption("未知错误"+e.getMessage(), UN_KNOWN_ERROR);
            return ex;
        }
    }
